...
- Knowledge intergration
	...
	- Bottom-up
		...
		- n개의 knowledge card $ C=\{c_1, ..., c_n\} $, query $ q $가 있을 때, 각각의 knowledge card로부터 $ n_1 $개의 document를 생성, $ \{d_1,...,d_{n \times n_1}\} $를 얻음

		- relevance selector를 사용해 $ n_2 $개의 relevant document를 선택, pruning selector로 pruning 실행, $ \{\tilde d_1, ..., \tilde d_{n_{2}} \} $를 얻음
