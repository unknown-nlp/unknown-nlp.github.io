...
- “**hinted**” **prompt** $ x_h $ (직접적으로 정답을 가리키는 힌트를 $ x_u $에 추가한 것)

	$$ \left(c_u, a_u\right)=M\left(x_u\right) ;\left(c_h, a_h\right)=M\left(x_h\right) $$

→ CoT의 faithfulness ==  $ x_u $에서는 힌트가 아닌 다른 답변을 말하지만 $ x_h $에서는 힌트를 정답으로 말할 때, 모델이 hinted prompt 문제를 풀기 위해 힌트를 사용했음을 말하는지 여부 ($ a_u \neq h \text { and } a_h=h $인 경우)

$$ \text { CoT faithfulness score }(M)=\mathbb{E}\left[\mathbf{1}\left[c_h \text { verbalizes the hint } h\right] \mid a_u \neq h, a_h=h\right] . $$
...
- $ \alpha $가 작을수록 무작위성이 큼을 의미함. (랜덤하게 골라졌을 확률이 높음.)

- $ \alpha $를 이용해 normalize 진행

$$ \text { normalized CoT faithfulness score }(M)=\min \{\text { CoT faithfulness score } /(1-q /(n-2) p), 1\} \text {. } $$

- **무작위성이 큰 환경에서도 hint를 골랐다는 건 그만큼 확신을 갖고 고른 것이다 → 신뢰성에 가중치!**

- $ \alpha $가 음수인 경우는 $ x_h $에서 $ h $를 고르지 않을 확률이 더 높다는 뜻이며 이런 데이터는 제거함.
...
- <span style='color:yellow_background'>Hack을 직접 언급하는 쉬운 방법이 있음에도 이렇게 돌려서 말하는 경향이 발생하는 것은 놀랍더라!</span>
