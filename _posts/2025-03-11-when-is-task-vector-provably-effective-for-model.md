...
- 주어진 **사전 학습 모델** \Psi^{(0)}와 여러 태스크 벡터 **{** {\Delta \Psi_{T_i}\}_{i \in V}}를 사용하여 **병합된 모델(merged model)** 을 구성할 수 있음:
  ...
  -  \Psi = \Psi^{(0)} + \sum_{i \in V} \lambda_i \Delta \Psi_{T_i} 
...
\Delta \Psi_{T'} = \Delta \Psi_{T_A} + (\Delta \Psi_{T_B} - \Delta \Psi_{T_C})
...
f(X; \Psi) = \frac{1}{P}\sum_{l=1}^P a^\top \text{ReLU}(W_O \sum_{l=1}^P W_V x \cdot \text{softmax} (x^\top W_K^\top W_Q x))
...
- **두 개의 이진 분류 태스크 T1, T2 를 고려**
  ...
  - 모델 병합:
\Psi = \Psi^{(0)} + \Delta \Psi_{T_1} + \lambda \Delta \Psi_{T_2}

  - 태스크 간의 상관관계( α ) 정의:
\alpha = \mu_{T_1}^\top \mu_{T_2} \in [-1,1]
...
선행 연구에 따르면, 태스크 벡터들은 서로 직교(orthogonal)하는 경향이 있음. 따라서, 본 연구에서는 각 태스크의 주요 특징을 나타내는 벡터들**\{ \mu_{T_i} \}_{i \in V_{\Psi}}****이 직교 정규 집합(orthonormal set)이라고 가정**

- 새로운 태스크 T′ 의 판별적 패턴:
\mu_{T'} = \sum_{i \in V_{\Psi}} \gamma_i \mu_{T_i} + \kappa \cdot \mu'_{\perp}
...
즉, **새로운 태스크 T′ 의 판별적 패턴은 기존 태스크의 패턴들의 선형 조합( ****\sum \gamma_i \mu_{T_i}****)과, 기존 태스크와 전혀 다른 새로운 패턴( ****\kappa \cdot \mu'_{\perp}****)으로 구성됨.**
...
\sum_{i \in V_{\Psi}} \lambda_i \gamma_i \geq 1 + c, \quad \sum_{i \in V_{\Psi}} \lambda_i \gamma_i^2 \geq 1 + c
...
|\lambda_i| \cdot \beta \leq c, \quad \forall i \in V_{\Psi}
...
- 저차원 근사 행렬을 ΔW_LR, ΔV_LR 라고 할 때, 다음이 성립함:
  ...
  - ||\Delta W_T - \Delta W_{LR}\|_F \leq M \cdot \epsilon + \frac{1}{\log M}
  ...
  - \|\Delta V_T - \Delta V_{LR}\|_F \leq \delta^{-1} \epsilon
